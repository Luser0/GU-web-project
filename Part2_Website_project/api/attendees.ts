...
const parseQuery = (query: QueryString): ParsedQuery => {
    ...
    const pageSize = Number.isInteger(rawPageSize) && rawPageSize > 0 && rawPageSize <= 100 ? rawPageSize : 25;
    return {
...
const getAttendees = async (parsed: ParsedQuery) => {
    ...
    const rows = await query.orderBy(attendee.id).limit(parsed.pageSize).offset(offset);
    return rows;
};
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    if (req.method === "GET") {
        try {
            ...
            return res.status(200).json({ data: rows, pagination: { page: parsed.page, pageSize: parsed.pageSize } });
        } catch (error) {
