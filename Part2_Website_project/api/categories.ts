...
const parseBooleanFlag = (value: string | string[] | undefined): boolean => {
...
const getCategories = async (query: QueryString) => {
  ...
  if (includeCounts) {
    ...
    const filteredQuery =
      searchTerm === ""
        ? baseQuery
        : baseQuery.where(like(event_category.name, `%${searchTerm}%`));
    const rows = await filteredQuery
      .groupBy(event_category.id)
      .orderBy(asc(event_category.name));
    return rows.map((row) => ({
      id: row.id,
      name: row.name,
  ...
  const filteredQuery =
    searchTerm === ""
      ? baseQuery
      : baseQuery.where(like(event_category.name, `%${searchTerm}%`));
  const rows = await filteredQuery.orderBy(asc(event_category.name));
  return rows;
};
